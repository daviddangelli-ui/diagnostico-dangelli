...
# --- OPÇÃO 2: GOVERNANÇA, ESTRATÉGIA E VALUATION (DETALHADO) ---
def diagnostico_valuation_ma():
    ...
    tabs = st.tabs(["🏛️ Governança e Longevidade", "🛡️ Proteção Patrimonial", "🎯 Estratégia", "💰 Valuation"])
    ...
    with tabs[0]:
        ...
        g5 = st.slider("Código de Ética formalizado e assinado por todos?", 1, 5, 1)
        g6 = st.slider("Regras claras para sucessão por morte e exclusão de sócios?", 1, 5, 1)
        g7 = st.slider("Política definida de distribuição de lucros?", 1, 5, 1)

    with tabs[1]:
        st.subheader("Proteção Patrimonial")
        p1 = st.slider("Holding constituída para o negócio e para cada acionista?", 1, 5, 1)
        p2 = st.slider("Acordo de Acionistas para a Holding e núcleos familiares?", 1, 5, 1)
        p3 = st.slider("Ativos protegidos contra riscos cíveis e cláusulas de impenhorabilidade?", 1, 5, 1)
        p4 = st.slider("Seguros contratados (Sucessão e Seguro D&O)?", 1, 5, 1)

    with tabs[2]:
        st.subheader("Estratégia")
        e1 = st.slider("Planejamento Estratégico (Mapa, Metas e Iniciativas) para 5 anos?", 1, 5, 1)
        e2 = st.slider("Reuniões mensais de acompanhamento de indicadores e KPIs?", 1, 5, 1)
        e3 = st.slider("Monitoramento mensal de DRE, Balanço e EBITDA?", 1, 5, 1)
        e4 = st.slider("Ações estratégicas alinhadas às necessidades da Reforma Tributária?", 1, 5, 1)

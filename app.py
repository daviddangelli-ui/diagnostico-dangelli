...
def diagnostico_valuation_ma():
    ...
    with t1:
        ...
        g3 = st.slider("Plano de sucessão e programa de formação?", 1, 5, 1)
        g4 = st.slider("Existe Conselho Consultivo e de Família?", 1, 5, 1)
        g5 = st.slider("Código de Ética assinado por todos?", 1, 5, 1)
        g6 = st.slider("Regras de sucessão por morte e exclusão?", 1, 5, 1)
        g7 = st.slider("Política de distribuição de lucros definida?", 1, 5, 1)
    with t2:
        st.subheader("Proteção Patrimonial")
        p1 = st.slider("Holding para o negócio e acionistas?", 1, 5, 1)
        p2 = st.slider("Acordo de Acionistas na Holding e família?", 1, 5, 1)
    ...
    with t3:
        ...
        e2 = st.slider("Acompanhamento mensal de KPIs?", 1, 5, 1)
        e3 = st.slider("Monitoramento de DRE e EBITDA?", 1, 5, 1)
        e4 = st.slider("Ações alinhadas à Reforma Tributária?", 1, 5, 1)
    with t4:
        st.subheader("Valuation")
        v1 = st.slider("Fez Valuation nos últimos 2 anos?", 1, 5, 1)
        v2 = st.slider("Modelagem financeira para crescimento?", 1, 5, 1)
...
def diagnostico_corporativo_completo():
    ...
    nome = st.text_input("Executivo:")
    empresa = st.text_input("Empresa Analisada:")
    
    tabs = st.tabs(["Estratégia", "Financeiro", "Cadeia", "Governança", "Tecnologia"])
    
    with tabs[0]:
        c1 = st.slider("Agilidade para dois regimes simultâneos?", 1, 5, 1)
        c2 = st.slider("Perenidade sem benefícios fiscais?", 1, 5, 1)
        c3 = st.slider("Estrutura de decisão robusta?", 1, 5, 1)
        c4 = st.slider("Cultura de reação rápida?", 1, 5, 1)
        c5 = st.slider("Painel de cenários H1, H2, H3?", 1, 5, 1)
        c6 = st.slider("Orquestrador vs Obsoleto?", 1, 5, 1)
    with tabs[1]:
        f1 = st.slider("Impacto no Fluxo de Caixa?", 1, 5, 1)
        f2 = st.slider("Mapeamento de prejuízo em produtos?", 1, 5, 1)
        f3 = st.slider("Gestão preditiva de EBITDA?", 1, 5, 1)
        f4 = st.slider("Controle de Margem Líquida?", 1, 5, 1)
        f5 = st.slider("Orçamento para conformidade?", 1, 5, 1)
        f6 = st.slider("Integridade de dados em tempo real?", 1, 5, 1)
    with tabs[2]:
        v1 = st.slider("Defesa de preço por Proposta de Valor?", 1, 5, 1)
        v2 = st.slider("Elasticidade de preço do cliente?", 1, 5, 1)
        v3 = st.slider("Igualdade físico vs digital?", 1, 5, 1)
        v4 = st.slider("Plano para fornecedores tóxicos?", 1, 5, 1)
        v5 = st.slider("Eficiência logística real?", 1, 5, 1)
        v6 = st.slider("Proteção em contratos longos?", 1, 5, 1)
    with tabs[3]:
        g1 = st.slider("Segregação patrimônio vs risco?", 1, 5, 1)
        g2 = st.slider("Acordo de sócios para M&A?", 1, 5, 1)
        g3 = st.slider("Conselho com competência técnica?", 1, 5, 1)
        g4 = st.slider("Segurança em assinar balanços?", 1, 5, 1)
        g5 = st.slider("Sucessão preparada para crise?", 1, 5, 1)
        g6 = st.slider("Compliance total sem pontos cegos?", 1, 5, 1)
    with tabs[4]:
        x1 = st.slider("Musculatura técnica do time?", 1, 5, 1)
        x2 = st.slider("Plano de retenção de talentos?", 1, 5, 1)
        x3 = st.slider("Treinamento de re-alfabetização?", 1, 5, 1)
        x4 = st.slider("ERP como ativo estratégico?", 1, 5, 1)
...
elif op == "2. Governança & Valuation":
